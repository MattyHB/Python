...
def isValidNum(num: str) -> bool:
    """ Takes string paramater and checks if it's a valid number based on it having only numeric values and no more than one decimal.
    
    Preconditions: 
    Postconditions: 
        * Returns a bool value for 'num'
    """
    decCount = 0
    for c in num:
        if c == '.':
